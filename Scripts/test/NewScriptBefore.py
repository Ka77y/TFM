...
def ifPutRule(docs, rule):
    ...
    try:
        for doc in docs:
            number_precision_eval = 1
            while number_precision_eval <= 5:
                #current_doc_backup = getDocumentInformationApi(doc['id'])[0]
                current_doc = doc.copy()
                precision_before = matchTopics(current_doc, "log_test_before_v1.csv", rule, number_precision_eval)
                csv_precision = current_doc['id'] + "," + precision_before.split(";")[0] + "," + precision_before.split(";")[1] + "," + ' '.join(
                    rule) + "," + str(number_precision_eval)
                file = open("log_precision_results_before_v1.csv", 'a')
                file.write("\n" + csv_precision)
                file.close()
                number_precision_eval = number_precision_eval + 2
    except:
        pass
